  ...
  return coroutine.wrap(function()
    ...
    for _, file in ipairs(files) do
      if file:sub(1, n) == basename then
        local suffix = file:sub(n + 1):match("^-(%w+)$")
        if suffix then
          coroutine.yield(workspace_dir .. PATHSEP .. file, suffix)
        end
      end
    end
  end)
...
local function consume_workspace_file(project_dir)
  ...
  local expected = norm(project_dir)
  local found_workspace = nil

  for filename, suffix in workspace_files_for(project_dir) do
    local load_f = loadfile(filename)
    local workspace = load_f and load_f()
    if workspace and norm(workspace.path) == expected then
      if not found_workspace then
        found_workspace = workspace
      end
      -- Clean up all matching duplicates; the current one will be saved on exit
      os.remove(filename)
    end
  end
...
local function get_workspace_filename(project_dir)
  local function norm(p) return (p or ""):gsub("\\", "/"):lower() end
  local p = norm(project_dir)
  local hash = 0
  for i = 1, #p do
    hash = (hash * 31 + string.byte(p, i)) % 0xFFFFFFFF
  end
  local basename = common.basename(project_dir)
  return USERDIR .. PATHSEP .. "ws" .. PATHSEP .. basename .. "-" .. string.format("%08x", hash)
end
