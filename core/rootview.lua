...
function RootView:on_mouse_pressed(button, x, y, clicks)
  ...
  if node and (node.hovered_scroll_button or 0) > 0 then
...
function RootView:on_mouse_moved(x, y, dx, dy)
  ...
  -- If a child view or widget explicitly set a cursor during on_mouse_moved, honor it.
  -- Otherwise, determine cursor based on tabs, scroll buttons, dividers, or the active view default.
  if div and (self.overlapping_node and not self.overlapping_node.active_view:scrollbar_overlaps_point(x, y)) then
    core.request_cursor(div.type == "hsplit" and "sizeh" or "sizev")
  elseif self.overlapping_node and self.overlapping_node:get_scroll_button_index(x, y) then
    core.request_cursor("arrow")
  elseif tab_index and not core.cursor_change_req then
    core.request_cursor("arrow")
  elseif self.overlapping_node and not core.cursor_change_req then
...
function RootView:draw()
  ...
  if core.cursor_change_req then
    if core.cursor_change_req ~= core.current_cursor then
      system.set_cursor(core.cursor_change_req)
      core.current_cursor = core.cursor_change_req
    end
    core.cursor_change_req = nil
  end
end
